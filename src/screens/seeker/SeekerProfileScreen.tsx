...
export const SeekerProfileScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const uploadResume = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        const response = await api.uploadResume(user?.token, base64File, selectedFile.name);
        ...
        <View style={styles.section}>
          ...
          {uploadingResume ? (
            ...
            <View style={styles.uploadContainer}>
              ...
              <TouchableOpacity style={styles.uploadButton} onPress={() => document.getElementById('resume-input')?.click()}>
                <Text style={styles.uploadButtonText}>
                  {resumeUrl ? '📄 Change Resume' : '📤 Upload Resume (PDF)'}
                </Text>
              </TouchableOpacity>
            </View>
          )}
          {(resumeFileName || selectedFile) && (
            <Text style={styles.fileNameText}>
              {selectedFile ? selectedFile.name : resumeFileName}
            </Text>
          )}
        </View>
